...
class CwCDataset(Dataset):
	...
	def __init__(
		...
		if split == "train" and augment_dataset and augmentation_factor >= 1:
			...
			if load_dataset:
				...
				if dump_dataset:
					# save processed data; this is available for offline training 
	...
	def process_samples(self, lower, compute_diff=True, compute_perspective=True):
			...
			for j in range(len(self.jsons)):
				...
				try:
						...
						if not last_world_state:
							for i2 in range(len(observation["ChatHistory"])):
								...
								for block in built_config:
									chat_with_actions_history.append({"idx": i, "action": "putdown", "time_stamp":time_stamp, "type": block["type"], "built_config": built_config, "screenshot_path":screenshot, "full_log_dir":full_log_dir, "prev_config": None, "builder_position": builder_position, "last_action": last_action, "gold_placement_list": gold_placement_list, "gold_removal_list": gold_removal_list})
								chat_with_actions_history.append({"idx": i, "action": "chat", "time_stamp":time_stamp, "utterance": observation["ChatHistory"][i2].strip(), "built_config": built_config, "screenshot_path":screenshot, "full_log_dir":full_log_dir, "prev_config": None, "builder_position": builder_position, "last_action": last_action, "gold_placement_list": gold_placement_list, "gold_removal_list": gold_removal_list})
								# all utt in the same state are stored again, with the same idx and time_stamp
						else:
							...
							for action_type in delta:
								for block in delta[action_type]:
									chat_with_actions_history.append({"idx": i, "action": action_type, "time_stamp":time_stamp, "type": block["type"], "built_config": built_config, "screenshot_path":screenshot, "full_log_dir":full_log_dir, "prev_config": prev_config, "builder_position": builder_position, "last_action": last_action, "gold_placement_list": gold_placement_list, "gold_removal_list": gold_removal_list})

							if len(observation["ChatHistory"]) > len(last_world_state["ChatHistory"]):
								# if new utts are generated
								delta = (len(observation["ChatHistory"]) - len(last_world_state["ChatHistory"]))
								for i3 in range(len(last_world_state["ChatHistory"]), len(observation["ChatHistory"])):
									...
									chat_with_actions_history.append({"idx": i, "action": "chat", "time_stamp":time_stamp, "utterance": observation["ChatHistory"][i3].strip(), "built_config": built_config, "screenshot_path":screenshot, "full_log_dir":full_log_dir, "prev_config": prev_config, "builder_position": builder_position, "last_action": last_action, "gold_placement_list": gold_placement_list, "gold_removal_list": gold_removal_list})
									# the newly genereted utts could have the same idx and time_stamp
					...
					for i in range(len(chat_with_actions_history)):
						...
						# only keep turns that have chat messages.
						if elem['action'] != 'chat':
							continue
						...
						screenshot_path = elem["screenshot_path"]
						time_stamp = elem["time_stamp"]
						if time_stamp in time_stamp_list:
							# print('discard this dialogue turn because there is already one ctx-reply pair having the same time stamp')
							discard_ct += 1
							continue
						...
						target_image_path = join(full_log_dir, '0-Architect-chat.png') 
						# target_image_path = target_image_path.replace('.png', '.jpg')
		...
		print("{} ctx-reply pairs have been discarded because of having repeated time_stamp".format(discard_ct))
	...
	def check_screenshot(self):
		...
		filtered_data = []
		orig_num = len(self.samples)
		for sp in self.samples:
			if sp["screenshot_path"] is not None:
				filtered_data.append(sp)
		self.samples = filtered_data
		print("discarded {} samples".format(orig_num - len(self.samples)))
	...
	def __getitem__(self, idx):
		...
		if 'image' in self.model.strip().split('_'):
			image_path = sample['full_log_dir']
			image_path = image_path.replace('logs/B', 'screenshots/B') + '/' + sample["screenshot_path"]
			# image_path = image_path.replace('.png', '.jpg')
			image = Image.open(image_path)
			screen_input = self.transform(image)
			screen_input = torch.stack(list(screen_input), dim=0)


			target_image_path = sample['full_log_dir']
			target_image_path = join(target_image_path.replace('logs/B', 'screenshots/B') , '0-Architect-chat.jpg')
			# target_image_path = target_image_path.replace(sample["screenshot_path"], '0-Architect-chat.jpg')
			target_image = Image.open(target_image_path)
			target_image_input = self.transform(target_image)
			target_image_input = torch.stack(list(target_image_input), dim=0)
		else:
			screen_input = None
		...
		return (
			...
			RawInputs(next_actions, next_actions_gold, sample['time_stamp'], json_id=sample.get('json_id'), sample_id=sample.get('sample_id'), next_utterance_raw=sample.get('next_utterance_raw'), built_config_ss=built_config, gold_config_ss=gold_config, colors_to_all_actions=colors_to_all_actions), # raw inputs for downstream purposes
	...
	def collate_fn(self, data):
		...
		prev_utterances, built_configs, gold_configs, target_inputs, target_outputs, next_placements, next_removals, gold_placements, gold_removals, all_placements_counter, all_next_placements_counter, all_removals_counter, block_counters_spatial_tensors, last_action_bits, raw_inputs, built_configs_3d, gold_configs_3d, perspective_coord_reprs, built_config_type_dists, gold_config_type_dists, screen_shot, target_image = zip(*data)
		...
		if screen_shot[0] is not None:
			screen_shot = screen_shot[0].unsqueeze(0)
			target_image = target_image[0].unsqueeze(0)
		
		return (
			EncoderInputs(prev_utterances, prev_utterances_lengths, built_configs, built_config_lengths, gold_configs, gold_config_lengths, next_actions, next_actions_lengths, gold_actions, gold_actions_lengths, block_counters, block_counters_spatial_tensors, last_action_bits, built_configs_3d, gold_configs_3d, perspective_coord_reprs, built_config_type_dists, gold_config_type_dists, screen_shot, target_image),
...
class EncoderInputs:
	def __init__(self, prev_utterances, prev_utterances_lengths, built_configs, built_config_lengths, gold_configs, gold_config_lengths, next_actions, next_actions_lengths, gold_actions, gold_actions_lengths, block_counters, block_counters_spatial_tensors, last_action_bits, built_configs_3d, gold_configs_3d, perspective_coord_reprs, built_config_type_dists, gold_config_type_dists, image_inputs=None, target_image=None):
...
class RawInputs:
	...
	def __init__(self, next_actions_raw, gold_next_actions_raw, time_stamp, json_id=None, sample_id=None, next_utterance_raw=None, built_config_ss=None, gold_config_ss=None, colors_to_all_actions=None):
...
if __name__ == '__main__':
	...
	parser.add_argument('--saved_dataset_dir', default="/datadrive/saved_data/mc-architect/data/saved_cwc_datasets/lower-no_perspective_coords/", help='location of saved dataset')
...
def gather_sessions(self, data_dir, gold_configs_dir, split, json_type='postprocessed-observations'):
	"""
	You can this function to gather json files from different folders
	"""
	all_sessions = list(
			map(
				remove_empty_states,
				map(
					reorder,
					get_logfiles_with_gold_config(data_dir, gold_configs_dir, split, json_type=json_type)
				)
			)
		) 
