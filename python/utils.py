...
def get_logfiles_with_gold_config(data_path, gold_configs_dir, split=None, with_gold_config=True, from_aug_data=False, json_type='postprocessed-observations'):
	"""
	...
	for data_root_dir in all_data_root_dirs:
		if data_root_dir.strip().split('-')[0]!='data':
			continue
		...
		for log_dir in all_log_dirs:
			...
			logfile = join(logs_root_dir, log_dir, json_type + ".json")
			with open(logfile) as f:
				loaded_json = json.loads(f.read())
				loaded_json["from_aug_data"] = from_aug_data
				loaded_json["full_log_dir"] = join(logs_root_dir, log_dir)
...
def get_screenshot_path(observation):
	"""
	Args:
		observation: The observations for a cetain world state

	Returns:
		The screenshot taken from the architect's view
	"""
	if "ScreenshotPath" in observation.keys():
		screenshot_path = observation["ScreenshotPath"]
	elif "Screenshots" in observation.keys():
		screenshot_path = observation["Screenshots"]['Builder']
...
if __name__ == "__main__":
	...
	get_augmented_data_splits(
		"/datadrive/saved_data/mc-architect/data/augmented-no-spatial/logs/", "/datadrive/saved_data/mc-architect/data/augmented-no-spatial/gold-configurations/", "../data/logs/splits.json"
	)
