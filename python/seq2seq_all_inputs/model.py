...
from seq2seq_world_state.model import NextActionsEncoder, BlockCountersEncoder, BlockRegionCountersEncoder,ImageEncoder, ImageEncoderDQN,WorldStateEncoderRNN
from seq2seq_world_state.feature_extractor import FeatureExtractor

class UtteranceAndImageCnnEncoder(nn.Module):
    ...
    def __init__(self, args, train_dl, vocab):
        super(UtteranceAndImageCnnEncoder, self).__init__()

        self.world_image_encoder = ImageEncoder(
            in_channels=3
        )
    ...
    def forward(self, encoder_inputs):
        # encoder_inputs, image_inputs = encoder_inputs_all
        image_state = self.world_image_encoder(encoder_inputs)
        world_state_hidden_final = self.world_state_encoder(encoder_inputs)
        print("Image encoder output: {}".format(image_state.shape))
        ...
        if self.rnn == "gru":
            input_encoding = torch.cat((image_state, world_state_hidden_final, prev_utterances_hidden_final), 2)
        elif self.rnn == "lstm":
            input_encoding = (
                torch.cat((image_state, world_state_hidden_final, prev_utterances_hidden_final[0]), 2),
                torch.cat((image_state, world_state_hidden_final, prev_utterances_hidden_final[1]), 2)
            )
...
class AllInputsEncoder(nn.Module):
    ...
    def __init__(self, args, train_dl, vocab):
        ...
        self.prev_utterances_encoder = PrevUtterancesEncoder(
            vocab, args.rnn_hidden_size, args.num_encoder_hidden_layers, dropout=args.dropout_rnn if args.dropout_rnn is not None else args.dropout, linear_size=args.encoder_linear_size, nonlinearity=args.encoder_nonlinearity, rnn=args.rnn, bidirectional=args.bidirectional, train_embeddings=args.train_embeddings
        )
...
class ImageAndUtterancesAndBlockRegionCountersEncoder(nn.Module):
    """
        Integrated model -- combines an encoder RNN for encoding previous utterances with a regional block counters encoder (which comes with an optional global block counters encoder as well)
    """
    def __init__(self, args, train_dl, encoder_vocab):
        super(ImageAndUtterancesAndBlockRegionCountersEncoder, self).__init__()


        # self.world_image_encoder = ImageEncoder(in_channels=3)
        # self.target_image_encoder = ImageEncoder(in_channels=3)

        self.world_image_encoder = FeatureExtractor()
        self.target_image_encoder = FeatureExtractor()
    ...
    def forward(self, encoder_inputs):
        image_encoding = self.world_image_encoder(encoder_inputs, target=False)
        target_image_encoding = self.target_image_encoder(encoder_inputs, target=True)
        block_counters_encoding = self.block_region_counters_encoder(encoder_inputs).decoder_input_concat
    
        rnn_hidden = self.prev_utterances_encoder(encoder_inputs).decoder_hidden

        # print("image encoding size: {}".format(image_encoding.shape))
        # [1, 128]
        # print("block counters encoding size: {}".format(block_counters_encoding.shape))
        # [1, 1, 168]
        # print("text encoding size: {}".format(rnn_hidden.shape))
        # [1, 1, 300]
        # rnn_hidden = torch.cat((rnn_hidden, target_image_encoding.unsqueeze(0)), -1)
        block_counters_encoding = torch.cat((block_counters_encoding, image_encoding.unsqueeze(0), target_image_encoding.unsqueeze(0)), -1)
